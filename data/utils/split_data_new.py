...
subdir = os.path.join(dir, 'rem_user_data_new')
files = []
if os.path.exists(subdir):
    files = os.listdir(subdir)
if len(files) == 0:
    subdir = os.path.join(dir, 'sampled_data_new')
    if os.path.exists(subdir):
        files = os.listdir(subdir)
if len(files) == 0:
    subdir = os.path.join(dir, 'all_data_new')
    files = os.listdir(subdir)
files = [f for f in files if f.endswith('_new.json')]
...
if (args.user):
    ...
    for f in files:
        ...
        ouf_dir_train = os.path.join(dir, 'train_new', file_name_train)
        ouf_dir_test = os.path.join(dir, 'test_new', file_name_test)
